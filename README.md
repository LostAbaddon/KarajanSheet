...
- **`vol:` 的 L/R 分声道渐变对单声道 TTS 段生效**：之前 TTS 输出为单声道，`vol:` 同时指定 L 与 R 时只有其中一个声道生效（且被错误地复制到两根声道）；现已自动把单声道升成立体声，L/R 渐变分别作用于左/右声道，未指定的声道保持 1.0。**注意**：旧脚本中单独使用 `vol: L0.5` / `vol: R0.8` 的单声道段会从单声道变成立体声（指定声道按值、另一声道 1.0），如需整体缩放请改用 `vol: A0.5`
